...
curl_hsm -X POST "$URL/hsm/$MODULE/$SLOT/generate/rsa" -d "{\"label\": \"$SIGNING_KEY_ID\", \"bits\": 4096}"; echo

echo "Generating HMAC hashing key: $HASHING_KEY_ID"
curl_hsm -X POST "$URL/hsm/$MODULE/$SLOT/generate/secret" -d "{\"label\": \"$HASHING_KEY_ID\", \"bits\": 256}"; echo
